...
XSOAR_STATUS_TO_CH: dict[int, str] = {
...
def fetch_incidents(
    ...
    filter_string = json.dumps(
        {
            "severity": sorted(valid_severity_filter),
            "status": sorted(valid_status_filter),
        },
        sort_keys=True,
    )
        ...
        incidents.append(
            {
                ...
                "closeReason": (
                    CLOSE_REASON_REVERSE_MAPPING.get(raw.get("close_reason", ""), raw.get("close_reason", ""))
...
def cyberhaven_incident_list_command(client: Client, args: dict[str, Any]) -> list[CommandResults] | CommandResults:
    ...
    next_page_readable = tableToMarkdown(
        ...
        [
            {
                "Next Page ID": page_response.get("next_id", ""),
                "Total": page_response.get("total", ""),
            }
        ],
...
def cyberhaven_incident_update_command(client: Client, args: dict[str, Any]) -> CommandResults:
    ...
    readable = build_incident_readable(
        resources,
        client._base_url,
        f"Incident **{params.get('incident_id')}** updated successfully.",
    )
